...
instacart %>% 
  ...
  mutate(summarise(n_obs = n(product_name))) %>%
  order(min_rank(desc(n_obs)))
  
```

Make a plot that shows the number of items ordered in each aisle, limiting this to aisles with more than 10000 items ordered. Arrange aisles sensibly, and organize your plot so others can read it. scatter ordered by magniture
